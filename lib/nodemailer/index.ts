...
export const sendWelcomeEmail = async ({email, name, intro}: WelcomeEmailData) => {
    const htmlTemplate = WELCOME_EMAIL_TEMPLATE.replace('{{name}}', name).replace('{{intro}}', intro).replace('{{email}}', email).replace('{{intro}}', intro);


    const mailOptions = {
        from: `"Signalist <signalist@elite.com>"`,
        to: email,
...
export const sendNewsSummaryEmail = async ({email, date, newsContent}: {
    ...
    const mailOptions = {
        from: `"Signalist Mews" <signalist@elite.com>`,
        to: email,
